...
const StadiumTicket = () => {
  ...
  const [matchStruct, setMatchStructSetter] = useState([
    "    struct MatchEvent {",
    "        string homeTeam;",
    "        string awayTeam;",
    "        uint256 startDate;",
  ...
  const hasDirectionAssign =
    "        categories[tokenId] = StadiumCategory(tokenId, price, BlockDirection(direction), capacity, name, 0);";
  const withoutDirectionAssign =
    "        categories[tokenId] = StadiumCategory(tokenId, price, capacity, name, 0);";
  const checkEventPassFunc = [
  ...
  const [matchDetailsCheck, setMatchDetailsCheck] = React.useState([
    true,
    true,
    true,
  ]);
  ...
  const [matchDetailTypes, setMatchDetailTypes] = React.useState([
    "string",
    "string",
    "uint256",
  ]);
  ...
  const addDirection = (e) => {
    if (e.key == "Enter" && targetDirectionName.length > 0) {
      if (userDirections.includes(targetDirectionName)) {
  ...
  const addMatchDetail = (e) => {
    ...
    if (matchDetails.includes(matchDetailName)) {
      alert("Please enter a unique name");
      return;
    }
    ...
    if (
      matchDetailName == "string" ||
      matchDetailName == "string[]" ||
      matchDetailName == "uint256[]" ||
      matchDetailName == "uint256" ||
      matchDetailName == "bool" ||
      matchDetailName == "address" ||
      matchDetailName == "bytes" ||
      matchDetailName == "uint128" ||
      matchDetailName == "uint64" ||
      matchDetailName == "uint32" ||
      matchDetailName == "uint16"
    ) {
  ...
  const checkMatchDetails = (event, index) => {
    const newCheckedMatchList = [
      ...matchDetailsCheck.slice(0, index),
      event.target.checked,
      ...matchDetailsCheck.slice(index + 1),
    ];
  ...
  const checkUserDirection = (event, index) => {
    const newCheckedDirectionList = [
      ...checkedDirections.slice(0, index + 4),
      event.target.checked,
      ...checkedDirections.slice(index + 5),
    ];
    setCheckedDirections(newCheckedDirectionList);
  ...
  const checkE = (event) => {
    const newCheckedDirectionList = [
      ...checkedDirections.slice(0, 1),
      event.target.checked,
      ...checkedDirections.slice(2),
    ];
    setCheckedDirections(newCheckedDirectionList);
    setDirections(newCheckedDirectionList);
  };
  ...
  const checkS = (event) => {
    const newCheckedDirectionList = [
      ...checkedDirections.slice(0, 2),
      event.target.checked,
      ...checkedDirections.slice(3),
    ];
    setCheckedDirections(newCheckedDirectionList);
    setDirections(newCheckedDirectionList);
  };
  ...
  const checkW = (event) => {
    const newCheckedDirectionList = [
      ...checkedDirections.slice(0, 3),
      event.target.checked,
      ...checkedDirections.slice(4),
    ];
    setCheckedDirections(newCheckedDirectionList);
    setDirections(newCheckedDirectionList);
  };
  ...
  const setDirections = (
    newCheckedList,
    userDirectionsParam = userDirections
  ) => {
  ...
  const setNewContract = (
    ...
    for (let i = 0; i < beforeLines.length; i++) {
      ...
      if (i === 0) {
        newLines.push(
          `contract ${newContractName} is ERC1155, Pausable, Ownable {`
        );
      } else if (i === 1) {
        if (hasDirection) {
        ...
        let lastLine = hasMatch
          ? newConstructorList[0]
          : withoutMatchConstructor;
        lastLine =
          lastLine.slice(0, lastLine.lastIndexOf("{") - 3) +
          targetURIParam +
          '") {';
        if (hasMatch) {
        ...
        if (hasMatch && newCheckedList[2]) newLines.push(requireLine);
        if (hasDirection) {
          const commaIndex = directions.lastIndexOf(",");
          const openIndex = directions.lastIndexOf("{");
          const lastDirection = directions.slice(
            (commaIndex === -1 ? openIndex - 1 : commaIndex) + 2,
            directions.lastIndexOf("}")
          );
          newLines.push(
            `        require(direction <= uint8(BlockDirection.${lastDirection}), "Direction is out of range.");`
          );
        }
      } else if (i === 6) {
        newLines.push(
          hasDirection ? hasDirectionAssign : withoutDirectionAssign
        );
      } else if (i === 7) {
        if (hasMatch && newCheckedList[2]) newLines.push(requireLine);
      } else if (i === 8) {
        if (hasMatch && newCheckedList[2])
          newLines = [...newLines, ...checkEventPassFunc];
      } else if (i === 9) {
        if (hasMatch && newCheckedList[2]) newLines.push(requireLine);
      }
    }
  ...
  for (let i = 0; i < userDirections.length; i++) {
    customCheckboxes.push(
      <CustomCheckbox
        label={userDirections[i]}
        onChange={(event) => checkUserDirection(event, i)}
      />
    );
  }
  ...
  for (let i = 0; i < matchDetails.length; i++) {
    matchCheckboxes.push(
      <CustomCheckbox
        label={matchDetails[i] + " - " + matchDetailTypes[i]}
        onChange={(event) => checkMatchDetails(event, i)}
      />
    );
  }

  const setMatchStruct = (
    newCheckedList,
    newMatchList = matchDetails,
    newMatchDetailTypes = matchDetailTypes
  ) => {
    ...
    startString =
      startString.slice(
        0,
        entered ? startString.length - 2 : startString.length
      ) + ') ERC1155("") {';
    ...
    for (let i = 0; i < newCheckedList.length; i++) {
      ...
      entered2 = true;
      newConstructorList.push(
        `            ${newMatchList[i]}: ${newMatchList[i]}_,`
      );
    }
    if (entered2) {
      const lastEl = newConstructorList[newConstructorList.length - 1];
      newConstructorList = [
        ...newConstructorList.slice(0, newConstructorList.length - 1),
