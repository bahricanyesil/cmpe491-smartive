...
const CafeMenu = () => {
  ...
  const [menuItemTypes, setMenuItemTypes] = useState(["BREAKFAST", "ENTREE", "SALAD", "PIZZA", "BURGER", "PASTA", "MEAT", "COLDDRINK", "HOTDRINK", "SPECIAL", "CAKE", "COOKIE", "BISKUIT", "PASTRY", "CANDY", "PUDDING", "DEEPFRIED", "FROZEN", "GELATIN", "FRUIT"]);
  ...
  const [menuItemProperties, setMenuItemProperties] = useState(['name', 'calories', 'preparationTime', 'ingredients']);
  const [menuItemPropertiesTypes, setMenuItemPropertiesTypes] = useState(['string', 'uint256', 'uint256', 'string[]']);
  const [menuItemPropertiesCheck, setMenuItemPropertiesCheck] = useState([true, true, true, true]);
  ...
  useEffect(() => {
    fetch(cafeMenuContract)
      ...
      .then(text => {
        ...
        setBeforeLines([allLines.slice(0, 8),allLines.slice(9, 12), allLines.slice(13, 19), allLines.slice(23, 37),allLines.slice(38, 41), allLines.slice(53, 85),allLines.slice(102)]);
      });
  }, []);
  ...
  const setMenuItemProperty = (checked, i) => {
    const newCheckedPropertyList = [...menuItemPropertiesCheck.slice(0, i), checked, ...menuItemPropertiesCheck.slice(i+1)];
    setMenuItemPropertiesCheck(newCheckedPropertyList);
    setNewContract(contractName, contractURI, menuItems, menuItemTypes, menuItemProperties, newCheckedPropertyList, menuItemPropertiesTypes);
  }
  ...
  const addNewMenuItemProperty = (e) => {
    ...
    if(newMenuItemProperty == 'string' || newMenuItemProperty == 'string[]' || newMenuItemProperty == 'uint256' || newMenuItemProperty == 'bool' || newMenuItemProperty == 'address' || newMenuItemProperty == 'bytes' || newMenuItemProperty == 'uint128' || newMenuItemProperty == 'uint64' || newMenuItemProperty == 'uint32' || newMenuItemProperty == 'uint16') {
    ...
    if(newMenuItemProperty.length > 0 && newMenuItemPropertyType.length > 0) {
      ...
      const newMenuItemProperties = [...menuItemProperties, newMenuItemProperty];
      setMenuItemProperties(newMenuItemProperties);
      const newMenuItemPropertiesTypes = [...menuItemPropertiesTypes, newMenuItemPropertyType];
      setMenuItemPropertiesTypes(newMenuItemPropertiesTypes);
      setNewMenuItemProperty('');
      setNewMenuItemPropertyType('');
      setNewContract(contractName, contractURI, menuItems, menuItemTypes, newMenuItemProperties, newMenuItemPropertiesCheck, newMenuItemPropertiesTypes);
    }
  }
  ...
  const addNewMenuItem = (e) => {
    ...
    if(newMenuItem.length > 0) {
      ...
      setNewMenuItem('');
      setNewContract(contractName, contractURI, newMenuItemsCheck, newMenuItemTypes);
    }
  }

  const setNewContract = (newContractName, targetURIParam, newMenuItemChecks, newMenuItems=menuItemTypes, newMenuItemProperties=menuItemProperties, newMenuItemPropertiesCheck=menuItemPropertiesCheck, newMenuItemPropertiesTypes=menuItemPropertiesTypes) => {
    let newLines = [];
    ...
    for(let i=0; i<beforeLines.length; i++) {
      ...
      if (i===0) {
        ...
        for(let j=0; j<newMenuItemProperties.length; j++) {
          ...
          newLines.push(`        ${newMenuItemPropertiesTypes[j]} ${newMenuItemProperties[j]};`);
        }
      } else if (i===3) {
        ...
        for(let j=0; j<newMenuItemProperties.length; j++) {
          if(!newMenuItemPropertiesCheck[j]) continue;
          if(newMenuItemPropertiesTypes[j].includes('string')){
        ...
        newLines.push('        require(price >= 0, "Price should be greater than or equal to 0.");');
        if(newMenuItemPropertiesCheck[1]) newLines.push('        require(calories > 0, "Calories should be greater than 0.");');
        if(newMenuItemPropertiesCheck[2]) newLines.push('        require(preparationTime > 0, "Preparation time should be greater than 0.");');
        if(newMenuItemPropertiesCheck[3]) newLines.push('        require(ingredients.length > 0, "Ingredients can not be empty.");');
        if(newMenuItemPropertiesCheck[0]) {
          newLines.push('        for(uint256 i=0; i<supplies.length; i++) {');
          newLines.push('            require(!compareStrings(menuItems[i].name, name), "There is already a menu item with the same name.");');
          newLines.push('        }');
        }
        newLines.push('        uint256 tokenId = _tokenIdCounter.current();');
        newLines.push('        _tokenIdCounter.increment();');
        let itemCreateLine = '        menuItems[tokenId] = MenuItem(tokenId, price, 0, MenuItemType(itemType)';
        for(let j=0; j<newMenuItemProperties.length; j++) {
          if(!newMenuItemPropertiesCheck[j]) continue;
        ...
        if(newMenuItemPropertiesCheck[2]) newLines.push('        require(preparationTime > 0, "Preparation time should be greater than 0.");');
        if(newMenuItemPropertiesCheck[3]) newLines.push('        require(ingredients.length > 0, "Ingredients can not be empty.");');
        newLines.push('        require(supplies.length > 0, "There is no item to update.");');
        newLines.push('        require(itemId <= supplies.length-1 && itemId >= 0, "Menu item does not exist.");');
        if(newMenuItemPropertiesCheck[0]) {
          newLines.push('        for(uint256 i=0; i<supplies.length; i++) {');
          newLines.push('            require(!compareStrings(menuItems[i].name, name), "There is already a menu item with the same name.");');
          newLines.push('        }');
        ...
        if(newMenuItemPropertiesCheck[0]) newLines.push('        menuItems[itemId].name = name;');
        newLines.push('        menuItems[itemId].itemType = MenuItemType(itemType);');
        if(newMenuItemPropertiesCheck[1]) newLines.push('        menuItems[itemId].calories = calories;');
        if(newMenuItemPropertiesCheck[2]) newLines.push('        menuItems[itemId].preparationTime = preparationTime;');
        for(let t=3; t<newMenuItemPropertiesCheck.length; t++) {
          if(!newMenuItemPropertiesCheck[t]) continue;
          newLines.push(`        menuItems[itemId].${newMenuItemProperties[t]} = ${newMenuItemProperties[t]};`);
        }
      }
    } 
  ...
  for(let i=0; i<menuItemProperties.length; i++) {
    ...
    menuItemPropertiesView.push(<CustomCheckbox id={item} label={item + ' - ' + itemType} onChange={(event) => setMenuItemProperty(event.target.checked, i)}/>);
  }
